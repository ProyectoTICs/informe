...
\title{Definición del problema y requerimientos}
%
...
\begin{abstract}
Se pretende en este documento indicar la planificación del proyecto que solicitó el cliente. En base a sus requerimientos debemos lograr un programa que algorítmicamente nos permita una establecer una equivalencia en el gasto incurrido entre compañeros de un trabajo en la compra de café, teniendo como una de las principales problemáticas a superar la variabilidad en la asistencia de los empleadosm los cuales no se ven involucrados en la transacción. Se busca también definir los requerimientos y formalizar el problema planteado.
\end{abstract}

\section{Introducción}

Este informe daremos los requerimientos necesarios para el desarrollo de la aplicación solicitada; en otras palabras, responderemos a la pregunta "¿Qué tiene que hacer el programa solicitando?" procurando dejar en claro el alcance y los roles, como tambien cada una de las funcionalidades necesarias para el caso.
Para poder generar los requerimientos, hay que tener en cuenta el problema objetivo: crear una distribución equitativa para repartir la compra de café en una empresa, donde la cantidad de personal que asiste nos es una constante durante el tiempo.
...
\section{NO ENTIENDO QUE ES ESTO!}
...
Finalmente, el equipo decidió ocupar la estrategia de desarrollo llamada "Scrum"; pequeñas reuniones en las que se comentan las desiciones. Se ocupará github como herramienta de administración de versiones, el cual se encargará de generar una especie de carpeta en cada computador personal, de manera que los cambios en un documento se vean reflejados en los archivos de cada integrante.

Para llevar la cuenta de las tareas completadas y las que estan aun en desarrollo (junto con las pendientes) se ocupará la aplicación "Trello" la cual es una forma simple de ver los "por hacer" y los "terminado".

Para terminar, se determinó que, con respecto al programa, este será destinado al conteo de tazas café compradas en una empresa, de manera que esta sea equitativa con respecto cantidad de tazas compradas y no a los costos; se asumirá que el precio por taza de café unitaria será fijo.
